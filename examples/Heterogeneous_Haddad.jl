...
function initialize_state(m::HaddadModel; μn = 30, vn = 30)
...
function initialize_y(m::HaddadModel, state)
  OrderedDict(:p => fill(1.0, length(state[:μ]), length(state[:v])))
end

function (m::HaddadModel)(state, y)
  μbar = m.μbar ; vbar = m.vbar ; κμ = m.κμ ; νμ = m.νμ ; κv = m.κv ; νv = m.νv ; αbar = m.αbar ; λ = m.λ ; ρ = m.ρ ; γ = m.γ ; ψ = m.ψ
  ...
  out2 = p * (1 / p + μc + μp - r - κ_Zc * σc - κ_Zμ * σp_Zμ - κ_Zv * σp_Zv)
  return (out,) , (μμ, μv), (p = p, r = r, κ_Zc = κ_Zc, κ_Zμ = κ_Zμ, κ_Zv = κ_Zv, αstar = αstar, out2 = out2)
end

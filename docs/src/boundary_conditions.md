...
boundary condition — on coarse grids this is noticeably more accurate.
...
[Writing the PDE function](pde_function.md#Pattern-2:-endogenous-drift-(consumption-saving)).
...
Pass the stopping payoff as a *lower bound* on the unknown with the `lower_bound` keyword

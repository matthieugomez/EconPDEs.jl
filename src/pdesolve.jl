...
========================================================================================#
# 1 state variable
@generated function derive(::Type{Tsolution}, grid::StateGrid{T1, 1, <: NamedTuple{N}}, y::AbstractArray{T}, icar, bc, drift = (0.0,)) where {Tsolution, T1, N, T}
    statename = N[1]
    expr = Expr[]
...
# 2 state variables
@generated function derive(::Type{Tsolution}, grid::StateGrid{T1, 2, <: NamedTuple{N}}, y::AbstractArray{T}, icar, bc, drift = (0.0, 0.0)) where {Tsolution, T1, N, T}
    statename1 = N[1]
    statename2 = N[2]
    expr = Expr[]
    for k in 1:length(Tsolution.parameters[1])
